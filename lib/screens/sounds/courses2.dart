...
class Courses2 extends StatefulWidget {
  static const String route='/Courses2';
  final String title;
  final String catId;

  const Courses2({Key? key, required this.title, required this.catId}) : super(key: key);
...
class _Courses2State extends State<Courses2> {
  ...
  getChildren()async{
    var children1=await MyChild().getChildren(url2: 'cources/${widget.catId}');
  ...
  Widget item({required double height,required double width,required String name,required String id}){
    return
      Padding(
        ...
        child: InkWell(
          onTap:(){
            Navigator.push(context, MaterialPageRoute(builder: (context) => AllSoundsScreen(title: name,catName: 'cources',catId: id),));
          },
          child: Container(
            height: height*.08,
            width: width,
