...
class Eqdia2 extends StatefulWidget {
  static const String route='/Eqdia2';
  final String title;
  final String catId;
  const Eqdia2({Key? key, required this.title, required this.catId}) : super(key: key);

  @override
  State<Eqdia2> createState() => _Eqdia2State();
}

class _Eqdia2State extends State<Eqdia2> {

  List<MyChild>? children;
  getChildren()async{
    var children1=await MyChild().getChildren(url2: 'munahaj_eaqdi/${widget.catId}');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        centerTitle: true,
        title:  Text( ' أقسام ${widget.title}',style: const TextStyle(fontSize: 20),),
  ...
  Widget item({required double height,required double width,required String name,required String id}){
    return
      Padding(
        ...
        child: InkWell(
          onTap:(){
            Navigator.push(context, MaterialPageRoute(builder: (context) => AllSoundsScreen(catId: id,title: name,catName:'munahaj_eaqdi' ,)));//id=cat_id
          },
